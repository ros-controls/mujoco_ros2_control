...
bool CameraPlugin::init(rclcpp::Node::SharedPtr node, const mjModel* model, mjData* /*data*/)
{
  ...
  service_ = node_->create_service<ApplyExternalWrench>(
      "apply_wrench", std::bind(&CameraPlugin::handleApplyWrench, this, std::placeholders::_1, std::placeholders::_2));

  RCLCPP_INFO(node_->get_logger(), "CameraPlugin initialised. Service available at '%s'.", service_->get_service_name());
...
{
  RCLCPP_INFO(node_->get_logger(), "CameraPlugin cleanup.");
  service_.reset();
  marker_pub_.reset();
...
void CameraPlugin::publish_markers(MarkerArray& markers) const
{
  if (active_wrenches_.empty())
  {
    return;
...
void CameraPlugin::publish_markers()
{
  MarkerArray msg;
  publish_markers(msg);
...
void CameraPlugin::handleApplyWrench(const ApplyExternalWrench::Request::SharedPtr request,
