...
std::string safeObjName(const mjModel * model, int obj_type, int id,
...
bool ConstraintViolationsPublisherPlugin::init(rclcpp::Node::SharedPtr node,
                                               const mjModel * model,
                                               mjData * /*data*/)
{
  node_ = node;
  ...
  if (rate_hz <= 0.0) {
    RCLCPP_WARN(node_->get_logger(),
                "publish_rate must be > 0, ignoring value %.1f and using 50 Hz.", rate_hz);
    rate_hz = 50.0;
  }
  publish_period_ = rclcpp::Duration::from_seconds(1.0 / rate_hz);

  publisher_ = node_->create_publisher<mujoco_ros2_control_msgs::msg::ConstraintViolations>(
...
std::string ConstraintViolationsPublisherPlugin::buildConstraintName(
  const mjModel * model, int eq_id) const
{
  ...
  switch (eq_type) {
    case mjEQ_JOINT: {
      const std::string n1 = safeObjName(model, mjOBJ_JOINT, obj1id,
                                          "joint" + std::to_string(obj1id));
      const std::string n2 = safeObjName(model, mjOBJ_JOINT, obj2id,
                                          "joint" + std::to_string(obj2id));
      return n1 + "_" + n2 + "_constraint";
    }
    case mjEQ_TENDON: {
      const std::string n1 = safeObjName(model, mjOBJ_TENDON, obj1id,
                                          "tendon" + std::to_string(obj1id));
      const std::string n2 = safeObjName(model, mjOBJ_TENDON, obj2id,
                                          "tendon" + std::to_string(obj2id));
      return n1 + "_" + n2 + "_constraint";
    }
    case mjEQ_CONNECT: {
      // obj1id / obj2id are body IDs; body 0 is the world body.
      const std::string n1 = safeObjName(model, mjOBJ_BODY, obj1id,
                                          "body" + std::to_string(obj1id));
      const std::string n2 = (obj2id == 0)
        ? "world"
        : safeObjName(model, mjOBJ_BODY, obj2id, "body" + std::to_string(obj2id));
...
std::string ConstraintViolationsPublisherPlugin::constraintTypeString(
  const mjModel * model, int eq_id) const
{
  switch (model->eq_type[eq_id]) {
    case mjEQ_JOINT:   return "joint";
    case mjEQ_TENDON:  return "tendon";
    case mjEQ_WELD:    return "weld";
    case mjEQ_CONNECT: return "connect";
...
PLUGINLIB_EXPORT_CLASS(
  mujoco_ros2_control_plugins::ConstraintViolationsPublisherPlugin,
