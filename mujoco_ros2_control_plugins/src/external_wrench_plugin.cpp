...
bool ExternalWrenchPlugin::init(
  rclcpp::Node::SharedPtr node, const mjModel * model, mjData * /*data*/)
{
  ...
  service_ = node_->create_service<ApplyExternalWrench>(
    "apply_wrench",
    std::bind(
      &ExternalWrenchPlugin::handleApplyWrench, this,
      std::placeholders::_1, std::placeholders::_2));

  RCLCPP_INFO(
    node_->get_logger(),
    "ExternalWrenchPlugin initialised. Service available at '%s'.",
...
void ExternalWrenchPlugin::update(const mjModel * model, mjData * data)
{
  ...
  active_wrenches_.erase(
    std::remove_if(
      active_wrenches_.begin(), active_wrenches_.end(),
  ...
  for (const auto & w : active_wrenches_) {
    ...
    const mjtNum point_world[3] = {
      xpos[0] + xmat[0] * w.application_point[0] +
        xmat[1] * w.application_point[1] + xmat[2] * w.application_point[2],
      xpos[1] + xmat[3] * w.application_point[0] +
        xmat[4] * w.application_point[1] + xmat[5] * w.application_point[2],
      xpos[2] + xmat[6] * w.application_point[0] +
        xmat[7] * w.application_point[1] + xmat[8] * w.application_point[2]
    };
...
void ExternalWrenchPlugin::visualize(
  const mjModel * /*model*/, mjData * data, mjvScene * scene)
{
  if (!scene || active_wrenches_.empty()) {
    return;
  }

  // Colors matching MuJoCo's native perturbation arrows:
  //   force  → red   (mjvGeom.rgba)
  //   torque → cyan
  static const float kForceRgba[4]  = {1.0f, 0.0f, 0.0f, 0.7f};
  static const float kTorqueRgba[4] = {0.0f, 0.7f, 0.9f, 0.7f};
  static const mjtNum kShaftWidth   = 0.007;  // arrow shaft radius [m]
  ...
  for (const auto & w : active_wrenches_) {
    ...
    // --- force arrow --------------------------------------------------------
    const mjtNum f_mag = std::sqrt(
      w.force[0] * w.force[0] + w.force[1] * w.force[1] + w.force[2] * w.force[2]);

    if (f_mag > 1e-9 && scene->ngeom < scene->maxgeom) {
      const mjtNum scale = force_arrow_scale_ * f_mag;
      const mjtNum tip_f[3] = {
        p[0] + w.force[0] * force_arrow_scale_,
        p[1] + w.force[1] * force_arrow_scale_,
        p[2] + w.force[2] * force_arrow_scale_
      };
      mjvGeom * g = scene->geoms + scene->ngeom++;
      mjv_initGeom(g, mjGEOM_ARROW, nullptr, nullptr, nullptr, kForceRgba);
      mjv_makeConnector(
        g, mjGEOM_ARROW, kShaftWidth * (scale > 0.01 ? scale / 0.01 : 1.0),
        p[0], p[1], p[2],
        tip_f[0], tip_f[1], tip_f[2]);
      g->rgba[0] = kForceRgba[0];
      g->rgba[1] = kForceRgba[1];
      g->rgba[2] = kForceRgba[2];
      g->rgba[3] = kForceRgba[3];
    }

    // --- torque arrow -------------------------------------------------------
    const mjtNum t_mag = std::sqrt(
      w.torque[0] * w.torque[0] + w.torque[1] * w.torque[1] + w.torque[2] * w.torque[2]);

    if (t_mag > 1e-9 && scene->ngeom < scene->maxgeom) {
      const mjtNum scale = torque_arrow_scale_ * t_mag;
      const mjtNum tip_t[3] = {
        p[0] + w.torque[0] * torque_arrow_scale_,
        p[1] + w.torque[1] * torque_arrow_scale_,
        p[2] + w.torque[2] * torque_arrow_scale_
      };
      mjvGeom * g = scene->geoms + scene->ngeom++;
      mjv_initGeom(g, mjGEOM_ARROW, nullptr, nullptr, nullptr, kTorqueRgba);
      mjv_makeConnector(
        g, mjGEOM_ARROW, kShaftWidth * (scale > 0.01 ? scale / 0.01 : 1.0),
        p[0], p[1], p[2],
        tip_t[0], tip_t[1], tip_t[2]);
      g->rgba[0] = kTorqueRgba[0];
      g->rgba[1] = kTorqueRgba[1];
      g->rgba[2] = kTorqueRgba[2];
      g->rgba[3] = kTorqueRgba[3];
    }

    // (void)model;  // unused but kept for API symmetry
  }
}
...
void ExternalWrenchPlugin::handleApplyWrench(
  ...
  ApplyExternalWrench::Response::SharedPtr response)
{
  ...
  if (body_id < 0) {
    ...
    response->message =
      "Body '" + request->link_name + "' not found in MuJoCo model.";
  ...
  RCLCPP_INFO(
    ...
    request->link_name.c_str(), body_id,
    duration.seconds(),
    w.force[0], w.force[1], w.force[2],
    w.torque[0], w.torque[1], w.torque[2],
    w.application_point[0], w.application_point[1], w.application_point[2]);
  ...
  response->message =
    "Wrench applied on body '" + request->link_name + "'.";
}
...
PLUGINLIB_EXPORT_CLASS(
  mujoco_ros2_control_plugins::ExternalWrenchPlugin,
